...
however long that takes to the length of the outage. Triage during an outage is
its own subject;
...
## Why adding a second provider later does not work
...
The wider decision, including when a single gateway remains correct, is covered
above, and the jobs a payment layer has to cover either way are in
...
plugin](/integrations/), and {{ site.provider_floor }} providers sit behind it,
with routing, failover, idempotency,
