...
3. FAQ sections are single-sourced from a per-page data file `_data/integrations/<slug>.yml` (a `faqs:` list of `q:`/`a:` pairs; answers may contain inline HTML). Render where the FAQ section belongs with `{% include faq.html items=site.data.integrations.<slug>.faqs %}` — the include emits both the visible accordion (wired to the shared `assets/js/product-page.js` `toggleFaq`) and the FAQPage JSON-LD from that one list, so never hand-write FAQ markup or FAQ schema. Optional include params: `eyebrow`, `bg`, `first_open`. If the slug contains a hyphen, dot-access fails in an include tag — assign first: `{% assign f = site.data.integrations["my-slug"].faqs %}{% include faq.html items=f %}` (see `integrations/phoca-cart/index.html`).
